...
export default function WorkingWithArrays() {
  ...
  const [todo, setTodo] = useState({
    ...
    const updateDescription = (newDescription: string) => {
        axios.post(`${API}/${todo.id}/description/${newDescription}`)
            .then(response => alert(response.data.message))
            .catch(error => console.error("Failed to update description", error));
    };

    const updateCompleted = (newCompleted: boolean) => {
        axios.post(`${API}/${todo.id}/completed/${newCompleted}`)
            .then(response => alert(response.data.message))
            .catch(error => console.error("Failed to update completed status", error));
    };
    ...
    <div id="wd-working-with-arrays">
      ...
      <br /><br />
        <input
            ...
            onBlur={() => updateDescription(todo.description)}
